...
#include <fstream>
#include <sstream>
int Test(const std::string& mapPath, const std::string& scenPath, const std::string& durationPath,double time_limit);
...
int main(int argc, char* argv[]) {
    if (argc != 5 && argc != 6) {
        std::cerr << "Usage: " << argv[0] << " <map_path> <scen_path> <duration_path> <runtime>" << std::endl;
        return -1;
    }
    ...
    return Test(mapPath, scenPath, durationPath, runtime);
}


int Test(const std::string& mapPath, const std::string& scenPath, const std::string& durationPath,double time_limit) {
    std::cout << "####### PP_SIPP Begin #######" << std::endl;
